...
class Utils {
    ...
    public static function generateSignature($clientId, $clientSecretKey, $resourceUrl, $httpMethod, $timestamp, $nonce, $transactionParams) 
    {
    ...
    public static function generateAccessToken($clientId, $clientSecret, $passortUrl) 
    {
...
//        $json = json_decode($curl_response, true);
    ...
    public static function generateSecureData($options, $pinData)
    {
        $pinBlock = '0000';
        $expiry = '0000';
    ...
    public static function getPinBlock($pin, $cvv2, $expiryDate, $pinKey, $randNum)
    {
        if(empty($pin)) {
            $pin = "0000";
        }
    ...
    public static function getMacData($app, $options) 
    {
        $macData = "";
        if (empty($app)) {
            return $macData;
    ...
    public static function getMac($macData, $macKey)
    {
        //do hmac here
    ...
    public static function getSecure($options, $app, $isActivate = false) 
    {
    ...
    public static function padRight($value, $maxLen) {
        $maxLength = $maxLen;
        $stringValue = $value;
    ...
    public static function padLeft($value, $maxLen) {
        $maxLength = $maxLen;
        $stringValue = $value;
        if (empty($stringValue) || strlen($stringValue) >= $maxLength) {
            return $stringValue;
    ...
    public static function print_array($var)
    {
        echo "<br>";
    ...
    public static function asc2bin($asc) 
    {
        $result = '';
        $len = strlen($asc);
        for ($i = 0; $i < $len; $i++)
    ...
    */
    public static function hexDecode($data)
    {
        $out = "";
        $end = strlen($data);
        while($end > 0)
    ...
    public static function decodingTable($b)
    {
        if($b == 'a')
            return '10';
        if($b == 'b')
    ...
    public static function ignore($c)
    {
